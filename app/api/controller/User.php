...
class User extends Common
{
    ...
    public function Center()
    {
        ...
        // 已兑换礼品卡密数量（giftcard插件, 插件未启用时安全返回0）
        $exchange_coupon_count = 0;
        if(!empty($this->user))
        {
            $gc_ret = CallPluginsServiceMethod("giftcard", "CardSecretService", "UserExchangeTotal", $this->user["id"]);
            if(!empty($gc_ret) && isset($gc_ret["code"]) && $gc_ret["code"] == 0)
            {
                $exchange_coupon_count = intval($gc_ret["data"]);
            }
        }
