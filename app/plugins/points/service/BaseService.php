...
class BaseService
{
    ...
    public static function BaseConfig($is_cache = true, $is_goods = false)
    {
        ...
        if($is_goods === true)
        {
            ...
            // 优先取商品管理勾选“首页显示”的可兑换商品；一个都未勾选时回退到应用配置的选品（兼容旧数据）
            $home_goods_ids = Db::name('Goods')->where([
                ['is_delete_time', '=', 0],
                ['is_shelves', '=', 1],
                ['plugins_points_exchange_integral', '>', 0],
                ['plugins_points_is_home_show', '=', 1],
            ])->order('id desc')->column('id');
            if(empty($home_goods_ids) && !empty($ret['data']['goods_exchange']) && is_array($ret['data']['goods_exchange']))
            {
                $home_goods_ids = array_column($ret['data']['goods_exchange'], 'gid');
            }
            if(!empty($home_goods_ids))
            {
                $res = self::GoodsList($home_goods_ids);
                if($res['code'] == 0 && !empty($res['data']) && !empty($res['data']['goods']))
                {
                    $goods_data = array_column($res['data']['goods'], null, 'id');
                    foreach($home_goods_ids as $gid)
                    {
            ...
            // 附加商品分类名称（供前端“分类筛选”标签使用）
            $ret['data']['goods_exchange_data'] = self::GoodsCategoryNamesHandle($ret['data']['goods_exchange_data']);
        }
        return $ret;
    }

    /**
     * 全量可兑换商品列表（分页，供前端商品搜索列表；不受“首页显示”勾选限制）
     * @param   [array]          $params [输入参数 page/limit/wd]
     */
    public static function ExchangeGoodsAllList($params = [])
    {
        $page = empty($params['page']) ? 1 : max(1, intval($params['page']));
        $limit = empty($params['limit']) ? 20 : min(50, max(1, intval($params['limit'])));
        $where = [
            ['is_delete_time', '=', 0],
            ['is_shelves', '=', 1],
            ['plugins_points_exchange_integral', '>', 0],
        ...
        if(!empty($params['wd']))
        {
            $where[] = ['title', 'like', '%'.$params['wd'].'%'];
        }
        $goods_ids = Db::name('Goods')->where($where)->order('id desc')->page($page, $limit)->column('id');
        $result = [];
        if(!empty($goods_ids))
        {
            $res = self::GoodsList($goods_ids);
            if($res['code'] == 0 && !empty($res['data']) && !empty($res['data']['goods']))
            {
                $goods_data = array_column($res['data']['goods'], null, 'id');
                foreach($goods_ids as $gid)
                {
                    if(array_key_exists($gid, $goods_data))
                    {
                        $result[] = $goods_data[$gid];
                    }
                }
            }
        }
        return self::GoodsCategoryNamesHandle($result);
    }

    /**
     * 商品列表附加分类名称 category_names
     * @param   [array]          $goods_list [商品列表]
     */
    public static function GoodsCategoryNamesHandle($goods_list)
    {
        if(empty($goods_list))
        {
            return [];
        }
        $category_rows = Db::name('GoodsCategoryJoin')->alias('gcj')->join('goods_category gc', 'gc.id=gcj.category_id')->where([['gcj.goods_id', 'in', array_column($goods_list, 'id')]])->field('gcj.goods_id,gc.name')->select()->toArray();
        $category_map = [];
        foreach($category_rows as $cv)
        {
            $category_map[$cv['goods_id']][] = $cv['name'];
        }
        foreach($goods_list as &$gv)
        {
            $gv['category_names'] = empty($category_map[$gv['id']]) ? [] : array_values(array_unique($category_map[$gv['id']]));
        }
        unset($gv);
        return $goods_list;
    }
