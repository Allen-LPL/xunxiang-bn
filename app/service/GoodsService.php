...
class GoodsService
{
    ...
    public static function GoodsDataHandle($data, $params = [])
    {
        if(!empty($data))
        {
            ...
            // 供货商名称
            if(in_array('supplier_id', $keys))
            {
                $supplier_list = SupplierService::SupplierName(array_column($data, 'supplier_id'));
            }
            ...
            foreach($data as $k=>&$v)
            {
                ...
                {
                    $v['supplier_name'] = (!empty($supplier_list) && is_array($supplier_list) && array_key_exists($v['supplier_id'], $supplier_list)) ? $supplier_list[$v['supplier_id']] : '';
                    $v['supplier_goods_url'] = (APPLICATION == 'app') ? '/pages/goods-search/goods-search?supplier='.$v['supplier_id'] : MyUrl('index/search/index', ['supplier'=>$v['supplier_id']]);
                }
    ...
    public static function GoodsSave($params = [])
    {
        ...
        $is_supplier_id = !empty($required_fields) && isset($required_fields['supplier_id']) && $required_fields['supplier_id'] == 1;
        ...
        $p = [
            ...
            [
                ...
                'is_checked'        => $is_supplier_id ? null : 1,
                'error_msg'         => MyLang('common_service.goods.form_item_supplier_id_message'),
            ],
            [
                'checked_type'      => 'length',
    ...
    public static function AutoGoodsList($params = [])
    {
        ...
        // 供货商条件
        if(!empty($params['goods_supplier_ids']))
        {
            if(!is_array($params['goods_supplier_ids']))
            {
                $params['goods_supplier_ids'] = explode(',', $params['goods_supplier_ids']);
            }
            $params['where'][] = ['g.supplier_id', 'in', $params['goods_supplier_ids']];
        }
