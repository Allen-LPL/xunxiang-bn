...
class Common extends BaseController
{
    ...
    public function __construct()
    {
        ...
        //新增通过配置动态隐藏菜单
        /*$menu_none = Config::get('cache.menu_none');
        if (!empty($menu_none) && is_array($menu_none) && !empty($menu['admin_left_menu'])) {
            foreach($menu_none as $k=>$v){
                $menu_arr = $menu['admin_left_menu'][$k];
                if(in_array($menu_arr['id'], $v['parent'])){
                    unset($menu['admin_left_menu'][$k]);
                }else{
                    foreach($menu_arr['items'] as $k2=>$v2){
                        if(in_array($v2['id'], $v['son'])){
                            unset($menu['admin_left_menu'][$k]['items'][$k2]);
                        }else{
                            if(empty($v['son2'])) continue;
                            if(isset($v2['items'])){
                                foreach($v2['items'] as $k3=>$v3){ 
                                    if(in_array($v3['id'], $v['son2'])){
                                        unset($menu['admin_left_menu'][$k]['items'][$k2]['items'][$k3]);
                                    }
                                }
                            }
        ...
        //print_r($menu['admin_left_menu']);exit;
